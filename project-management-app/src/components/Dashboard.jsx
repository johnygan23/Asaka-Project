...
const projectsMock = [
  { id: 1, name: 'Website Redesign', role: 'Member', status: 'Active', color: 'bg-cyan-500' },
  { id: 2, name: 'Mobile App', role: 'Manager', status: 'Paused', color: 'bg-yellow-400' },
  { id: 3, name: 'API Development', role: 'Member', status: 'Completed', color: 'bg-green-500' },
];
...
const Dashboard = ({ onToggleSidebar }) => {
  ...
  const userName = 'Wong'; // Replace with real user data if available
  ...
  // Pie chart data
  const projectCompleted = projectsMock.filter(p => p.status === 'Completed').length;
  const projectPercent = projectsMock.length ? Math.round((projectCompleted / projectsMock.length) * 100) : 0;
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-lg p-0 border border-gray-200 flex flex-col items-center justify-center">
            ...
            <div className="flex-1 flex items-center justify-center py-6">
              <DonutChart
                ...
                incomplete={projectsMock.length - projectCompleted}
